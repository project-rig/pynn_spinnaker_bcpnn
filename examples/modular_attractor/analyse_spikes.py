...
def display_spikes(e_spikes, i_spikes, raster_axis, rate_axis, num_hcus, num_mcu_neurons,
    ...
    # Determine which minicolumn excitatory spikes have originated
    e_minicolumn = numpy.floor(e_spikes[:,0] / num_mcu_neurons)

    # Plot spike raster, colouring excitatory spikes based on minicolumn
    raster_axis.scatter(e_spikes[:,1], e_spikes[:,0], c=e_minicolumn / float(num_mcus), cmap=cmap, linewidths=0.0, s=scatter_size)
    if i_spikes is not None:
        raster_axis.scatter(i_spikes[:,1], i_spikes[:,0] + (NE * num_hcus), color="gray", linewidths=0.0, s=scatter_size)
    ...
    for m in range(num_mcus):
        ...
        rate_axis.plot(minicolumn_histogram[1][:-1], minicolumn_rate, color=pylab.cm.rainbow(float(m) / float(num_mcus)))
...
def combine_e_spikes(filenames, segment, interleave, num_mcu_neurons):
    ...
    for i, f in enumerate(filenames):
        ...
        if interleave:
            # Determine which minicolumn each
            hcu_e_minicolumn = numpy.floor(hcu_e_spikes[:,0] / num_mcu_neurons)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interleave", action="store_true", help="Should spikes from HCUs be interleaved")
    ...
    e_spikes = combine_e_spikes(e_filenames, 0, args.interleave, args.num_mcu_neurons)
